...
Today both migrations and runtime queries use the same Postgres role (`devtoolbox` locally; whatever role owns the database in Neon in production), which can run DDL. `scripts/create-least-privilege-db-role.sql` provisions a second `devtoolbox_app` role scoped to `SELECT/INSERT/UPDATE/DELETE` only, with default-privilege grants so future migrated tables inherit the same scoping automatically. This has to be run manually against the production database (Neon) by whoever holds the current admin/migration credential (Claude doesn't have access to the production Neon project) — via Neon's SQL Editor (console) or `psql "$NEON_DIRECT_URL" -f scripts/create-least-privilege-db-role.sql` from a terminal. The script itself documents the steps. Postgres moved off Render to a dedicated Neon project per app (no more shared-instance-with-another-app concern), but it's still good practice to double check with `\c devtoolbox` (or `\l` to see the exact name) that you're connected to the right database before running past the `GRANT CONNECT ON DATABASE` line.

Once the role exists with the right grants, `DATABASE_URL` (a Render env var on the `devtoolbox-api` web service, even though the value itself is now a Neon connection string) is repointed at `devtoolbox_app`'s connection string, and a new env var, `MIGRATE_DATABASE_URL`, holds the original admin role's Neon connection string. This second variable exists because Render's free-tier web service plan doesn't support `preDeployCommand` (`render.yaml`'s comment on this), so `prisma migrate deploy` runs as the first step of `backend/Dockerfile`'s container `CMD` on every boot, not just on deploys with a new migration — that command now explicitly runs under `MIGRATE_DATABASE_URL` (falling back to `DATABASE_URL` if unset, so this is backward compatible until the split is actually done), while the app process itself starts under `DATABASE_URL`. Skipping the `MIGRATE_DATABASE_URL` step after switching `DATABASE_URL` to the restricted role breaks every boot, since migrations would then run without DDL privileges. Use Neon's *direct* (non-pooled) connection string for both variables — see PROD_READY.md §6 step 3 for why.
