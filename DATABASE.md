...
Today both migrations and runtime queries use the same Postgres role (`devtoolbox` locally; whatever Render provisions in production), which can run DDL. `scripts/create-least-privilege-db-role.sql` provisions a second `devtoolbox_app` role scoped to `SELECT/INSERT/UPDATE/DELETE` only, with default-privilege grants so future migrated tables inherit the same scoping automatically. This has to be run manually against the production database by whoever holds the current admin/migration credential (Claude doesn't have access to Render's production database) — the script itself documents the steps, including a gotcha worth calling out here too: if this Postgres instance hosts more than one app's database (as it does — see `render.yaml`'s `databases:` note), running the script while connected to the wrong database silently grants these permissions on the wrong app's tables instead. Check with `\c devtoolbox` (or `\l` to see the exact name) before running past the `GRANT CONNECT ON DATABASE` line.

Once the role exists with the right grants, `DATABASE_URL` (Render env var) is repointed at `devtoolbox_app`'s connection string, and a new env var, `MIGRATE_DATABASE_URL`, holds the original admin role's connection string. This second variable exists because Render's free-tier web service plan doesn't support `preDeployCommand` (`render.yaml`'s comment on this), so `prisma migrate deploy` runs as the first step of `backend/Dockerfile`'s container `CMD` on every boot, not just on deploys with a new migration — that command now explicitly runs under `MIGRATE_DATABASE_URL` (falling back to `DATABASE_URL` if unset, so this is backward compatible until the split is actually done), while the app process itself starts under `DATABASE_URL`. Skipping the `MIGRATE_DATABASE_URL` step after switching `DATABASE_URL` to the restricted role breaks every boot, since migrations would then run without DDL privileges.
