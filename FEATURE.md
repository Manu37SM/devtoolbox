...
| Shareable tool state (short links) | P2 | ✅ Shipped Phase 3 backend, frontend + org branding added Phase 4 — Share Links module. The Phase 3 pass only shipped the backend; there was no Share button or public viewer anywhere in the app until the Phase 4 branding pass built `ShareButton` (wired into Pipelines + Snippets) and the public `/s/[slug]` viewer. See AUDIT_REPORT.md §22. |
| Pipeline sharing | P2 | ✅ Shipped Phase 3 backend, frontend Phase 4 — via Share Links, see above |
| Team workspaces | P3 | ✅ Shipped Phase 4 (MVP scope), email-invites + custom branding + SSO added as follow-ups — org CRUD, member roles (OWNER/ADMIN/MEMBER), shared snippets/pipelines, org AI-usage dashboard, TEAM-tier quota inheritance for members of a TEAM-owner's org, email-token invites, org branding (name + logo) on shared links via `PATCH /organizations/:id/branding`, and org-level SSO (OIDC + SAML, JIT-provisioned org membership) via `/organizations/:id/sso` + `/sso/*`. Nothing left deliberately deferred from the original team workspaces scope. See AUDIT_REPORT.md §17/§21/§22/§23. |
| Public API / CLI | P3 | |
...
- Pro tier groundwork (usage quotas, billing integration). **✅ Shipped Phase 4, migrated to Razorpay** — Razorpay subscriptions via a client-side Checkout.js modal (no card/UPI data touches this backend), `User.plan` synced from subscription webhooks + immediate post-payment verification, `/account` shows plan status and upgrade/cancel actions. Originally built on Stripe; migrated before any production subscription existed because Stripe doesn't support billing for this business from India. Usage quotas (`PlanThrottleGuard`, AI Gateway `AI_HOURLY_QUOTA`) already existed from earlier phases and needed no changes — this closes the "how does a user actually become PRO/TEAM" gap those quotas were waiting on. See AUDIT_REPORT.md §15/§20.

### Phase 4 — Enterprise/ecosystem

- Team workspaces, org AI quotas/admin dashboard. **✅ Shipped (MVP scope), email-token invites, custom branding, and SSO added as follow-ups** — activated the previously-unused `Organization`/`OrganizationMember` scaffolding: org CRUD, member roles, shared snippets/pipelines (`organizationId`, additive to normal ownership), an org AI-usage dashboard, and TEAM-tier quota inheritance via `resolveEffectivePlan()`. No separate org-level Razorpay subscription — an org is "active" purely because its owner personally holds a TEAM plan. `addMember` now sends a 7-day email-token invite (accept page at `/invites/:token`) instead of 404ing when no account exists yet for that email. `Organization.brandName`/`brandLogoUrl` let an OWNER set custom branding shown on that org's shared links; building this surfaced that Share Links (Phase 3) had never gotten a frontend, so the base Share UI shipped in the same pass. Org SSO (OIDC + SAML) shipped last, closing out the final deferred item — one `SsoConnection` per org, `OWNER`-configurable, JIT-provisions both the `User` and `OrganizationMember` rows on first sign-in. Two new dependencies (`jose`, `@node-saml/node-saml`) flagged and confirmed with the user before adding, per CLAUDE.md rule 10. Nothing left deliberately deferred. See AUDIT_REPORT.md §17/§21/§22/§23.
- Pro tier / billing. **✅ Shipped, migrated to Razorpay** — Razorpay Checkout.js modal + cancel-subscription API, `User.plan` synced from webhooks. See AUDIT_REPORT.md §15/§20.
