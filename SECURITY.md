...
DevToolbox is a free, AI-augmented developer tools platform. Most tools run entirely client-side and never send input to a server; the surfaces that do touch a server — accounts/sync, sharing, billing, the AI Gateway, org SSO, the plugin sandbox — are where a security report is most likely to matter.
...
We currently do not run a paid bug bounty program. Credit in the release notes is offered to reporters who want it, once a fix has shipped.

## Scope

**In scope:**
- The backend API (`backend/`) — auth, sync, sharing, billing, AI Gateway, organizations/SSO, public API, plugin marketplace
- The frontend web app (`frontend/`), including client-side tool transforms if a bug there could corrupt or leak data across the app's trust boundaries
...
- Denial-of-service via sheer traffic volume (report a *bypass* of rate limiting, not the fact that a large enough flood degrades service)
...
- Auth-relevant events (failed logins, lockouts, password resets, refresh-token reuse) are persisted to `SecurityEventLog` and logged structurally (`SecurityLogService`). Never includes tool input/output content.
...
- Org SSO client secrets and history-sync previews are encrypted at rest (AES-256-GCM, per-org/per-user key derivation).
- Org-level SSO connections are scoped to the email domain they claim; full DNS-based domain-ownership verification is a known, disclosed gap, not yet built — an org OWNER can still self-attest a domain for *new* account provisioning (not takeover of an existing account, which is blocked).
- The plugin marketplace executes third-party WASM inside a sandboxed, opaque-origin iframe with `sandbox="allow-scripts"` only and `connect-src 'none'`.
- No tool input/output content is ever sent to error tracking (Sentry) or analytics — enforced via manual, minimal exception reporting: no session replay, no default request-body capture.
