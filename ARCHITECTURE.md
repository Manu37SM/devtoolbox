...
- **Security-tool cryptography/parsing libraries:** `hash-wasm` (WASM bcrypt/Argon2id implementations, used by the bcrypt/argon2 Hash & Verify tool — chosen specifically for correctness parity with server-side libraries, per FEATURE.md's note for that tool, over a hand-rolled JS implementation of either algorithm) and `jsrsasign` (pure-JS X.509/ASN.1 parsing, used by the Certificate (PEM/CRT) Decoder tool — ASN.1 DER parsing is not something worth reimplementing by hand for one tool). Both flagged and approved per CLAUDE.md rule 10 before adding; both used entirely client-side, same code-split-per-tool policy as the libraries above.
...
- **Billing (`razorpay` SDK, Phase 4; migrated off `stripe` — AUDIT_REPORT.md §20):** the one approved payment-provider dependency — see §14.2. Stripe was the original choice but doesn't support billing for this business from India, so it was replaced with Razorpay before any production subscription existed. Webhook signature verification (HMAC-SHA256 of the raw request body against the webhook secret, verified manually via `node:crypto` rather than an SDK helper, since raw request body is required for that one route only) is this codebase's first instance of that pattern; every other inbound-request surface (Module 8's webhook-inbox tool) just captures/displays, it doesn't authenticate the sender.
- **Org SSO (`jose` + `@node-saml/node-saml`, Phase 4 follow-up — AUDIT_REPORT.md §23):** two new approved dependencies, both flagged and confirmed with the user before adding (CLAUDE.md rule 10) — `jose` for OIDC id_token JWKS fetch + RS256 verification, `@node-saml/node-saml` for SAML assertion XML signature verification. Both are genuinely security-sensitive cryptographic primitives, unlike the plain-fetch OAuth exchange `auth/oauth.service.ts` already hand-rolls for GitHub/Google (which only ever calls a provider's HTTPS API directly and never needs to verify a signed token itself).
...
- **Frontend:** static tool pages served from CDN edge (Vercel/Cloudflare) — scales to near-infinite read traffic with no origin load; all heavy compute is client-side (the user's own CPU), which means DevToolbox's own infra scales independently of tool usage volume. **✅ Deployed on Vercel** (`vercel.json` at repo root, deployed via `.github/workflows/deploy-frontend.yml`) — see AUDIT_REPORT.md §24.
- **Backend:** stateless NestJS pods behind a load balancer, horizontally autoscaled on CPU/request-rate; all shared state in Postgres/Redis, never in-process, so any pod can serve any request. **✅ Deployed on Render** (`backend/Dockerfile`, `render.yaml` — single free-tier web service at MVP scale; horizontal scaling is a plan/service-count change in `render.yaml`, not an architecture change, when traffic warrants it). Originally deployed on Fly.io — moved to Render (AUDIT_REPORT.md §25) once Fly.io dropped its free tier in 2026, since the app must stay free to run. See AUDIT_REPORT.md §24/§25.
- **Database:** managed Postgres on Neon (serverless Postgres, free tier), a separate provider from the Render-hosted API/Redis — moved off Render at the user's explicit direction (AUDIT_REPORT.md, dated infra entry), superseding the earlier same-account decision in §25. Read replicas / a paid Neon plan are the natural upgrade path when read traffic or free-tier limits become limiting; Neon's own pooler (PgBouncer-compatible, the `-pooler` connection string) is available if/when connection-count pressure warrants it — not used today since the app runs as a single long-running server, not serverless functions.
...
2. **Pro tier (optional, for teams/power users):** higher AI usage quotas, team workspaces with shared pipelines/snippets, SSO, priority support, custom branding for shared links. The free tier's AI quota is generous enough for individual daily use; Pro removes the ceiling for heavy/team use, not the floor for individuals.
   - **Team workspaces — implemented (Phase 4), MVP scope:** org CRUD, member roles (OWNER/ADMIN/MEMBER), shared snippets/pipelines (optional `organizationId`, additive to normal ownership — DATABASE.md §3), an org AI-usage dashboard, and TEAM-tier quota inheritance for members whenever the org's owner personally holds a TEAM plan (`resolveEffectivePlan()`, no separate org-level Razorpay subscription — see DATABASE.md's Organization model note). Email-token invite/accept flow shipped as a follow-up (AUDIT_REPORT.md §21) — members with an existing account are still added immediately, but anyone else now gets a 7-day invite link by email instead of a 404. Custom branding for shared links also shipped as a follow-up (AUDIT_REPORT.md §22): `Organization.brandName`/`brandLogoUrl` (OWNER-settable, API.md §8.1), shown on a share link's public page when the link was created with that org's `organizationId` — this also meant building the Share Links feature's frontend for the first time (a `Share` button on Pipelines/Snippets, a public `/s/:slug` viewer), since it had shipped backend-only in Phase 3 with no UI at all. Org SSO (OIDC + SAML) shipped last (AUDIT_REPORT.md §23) — the final deferred item from the original MVP scope, closing out this feature area. API.md §17.
   - **Implementation (Phase 4; migrated Stripe → Razorpay, AUDIT_REPORT.md §20):** Razorpay subscriptions, opened via Razorpay's client-side Checkout.js modal (no custom card/UPI-collection UI in this codebase, so PCI/payment-instrument scope stays minimal — the modal is Razorpay's own hosted UI, just embedded rather than a full-page redirect). PRO/TEAM prices are configured as Razorpay Plan IDs via env vars, not hardcoded rupee amounts in code, so pricing changes don't require a deploy. `User.plan` (already the single source of truth every plan check reads — `PlanThrottleGuard`, `ApiKeysService`, AI Gateway quotas) is kept in sync from Razorpay subscription webhooks (and, redundantly, an immediate post-payment verification call — API.md §9) rather than trusted from the client alone; a `Subscription` table (DATABASE.md §3) tracks the underlying Razorpay subscription state for support/debugging, separate from the denormalized `User.plan` flag every other module already depends on. Two structural deviations from the original Stripe design, both in AUDIT_REPORT.md §20: Razorpay has no hosted Checkout page (the frontend opens a modal instead of redirecting) and no hosted Customer Portal (cancellation is a direct API call, `POST /billing/cancel-subscription`, instead of a portal redirect).
