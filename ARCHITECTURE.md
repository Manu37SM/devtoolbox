...
- **Database:** managed Postgres on Render (`render.yaml`'s `databases:` block, free tier), same account as the backend service rather than a separate provider — see AUDIT_REPORT.md §25 for why. Read replicas / a paid instance type are the natural upgrade path when read traffic or the free tier's 30-day expiry become limiting; connection pooling via PgBouncer if/when needed.
